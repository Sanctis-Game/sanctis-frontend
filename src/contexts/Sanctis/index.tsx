...
import { BigNumber, utils, Contract, providers, ethers } from 'ethers'
...
export const SanctisProvider: React.FC = ({ children }) => {
  ...
  const contracts = useMemo(() => {
    ...
    return {
      ...
      fusionPlants: new Contract(deployedAddresses[chainId].fusionPlants, PowerPlantsInterface, signer),
  ...
  const fetchOwnedCommanders = useCallback(async () => {
    ...
    setOwnedCommanders(await Promise.all(Array(commandersBalance).fill(0).map(async (_, i) =>
      (await contracts.commanders.tokenOfOwnerByIndex(account, i)).toNumber()
    )))
  }, [account, contracts])
  ...
  const fetchCommander = useCallback(async (commanderId: number) => {
    if (!contracts) return
    ...
    try {
      ...
      const commanderPlanets = await Promise.all(
        Array(empireSize).fill(0).map(async (_, i) =>
          (await contracts.planets.commanderPlanetByIndex(commanderId, i)).toString()
        )
      )
      const commander = {
        id: commanderId,
  ...
  const fetchPlanet = useCallback(async (planetId: string) => {
    if (!contracts) return
    try {
      const [status, ruler, x, y, z, humidity] = await contracts.planets.planet(planetId)
      const reserves: Reserve[] = await Promise.all(approvedObjects.resources.map(async (resource) => {
        const ResourceInterface = new utils.Interface(ResourceABI.abi)
        const contract = new Contract(resource.address, ResourceInterface, new providers.Web3Provider(ethereum).getSigner())
        return {
          resource,
          amount: await contract.reserve(planetId)
        }
      }))
      const infrastructures = await Promise.all(approvedObjects.infrastructures.map(async infrastructure => {
        const InfrastructureInterface = new utils.Interface(InfrastructureABI.abi)
        const contract = new Contract(infrastructure.address, InfrastructureInterface, new providers.Web3Provider(ethereum).getSigner())
        return {
          ...infrastructure,
  ...
  const createCommander = useCallback(async (name: string, race: Race) => {
    if (!contracts) return
    open(async () => {
      try {
  ...
  const colonizePlanet = useCallback(async (commanderId: number, x: number, y: number, z: number) => {
    if (!contracts) return
